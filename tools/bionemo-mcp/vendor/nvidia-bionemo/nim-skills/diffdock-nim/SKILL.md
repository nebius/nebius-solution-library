...
name: diffdock-nim
description: >
  Run DiffDock molecular docking via NVIDIA NIM to predict small-molecule binding poses against protein targets. Use for DiffDock, molecular docking, ligand docking, blind docking, SMILES or SDF ligands, ranked poses, confidence scores, Nebius-hosted MCP, or Nebius-hosted MCP deployment.
...
1. Call `list_models` and confirm that `diffdock_dock` is registered. Its presence
   proves every required catalog model was enabled and ready when the server
   started.
2. Call `diffdock_dock`. Use the JSON schema published by MCP; do not construct an
...
Supply ATOM-only PDB text, ligand text, its file type, and docking controls.
...
but execute the request with `diffdock_dock` and retrieve files from its artifact
