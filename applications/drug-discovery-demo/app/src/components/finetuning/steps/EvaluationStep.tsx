 ...
 *
 * Fifth step: Evaluate the trained model performance.
 * Shows metrics, prediction plots, and comparison with baseline.
 */

import { useMemo, useEffect } from 'react';
import { useFineTuning } from '../../../contexts/FineTuningContext';
import type { EvaluationResult } from '../../../types/finetuning';
...
export function EvaluationStep() {
  ...
  useEffect(() => {
    ...
    const predictions = Array.from({ length: numPoints }, (_, i) => {
      const actual = Math.random() * 4 - 1; // -1 to 3 (log10 scale)
      const noise = (Math.random() - 0.5) * 0.8;
      const predicted = actual + noise;
      return {
        actual,
        predicted,
        smiles: `C${i}CC(=O)O`, // Placeholder
        residual: predicted - actual,
      };
    });

    // Sort by actual for nice visualization
  ...
  }, [trainingResult, evaluationResult, setEvaluationResult]);
  ...
  const qualityAssessment = useMemo(() => {
    ...
    if (r2 >= 0.8 && mae <= 0.4) {
      return {
        level: 'excellent',
        message: 'Excellent model performance! Ready for virtual screening.',
        color: 'var(--color-success)',
      };
    } else if (r2 >= 0.6 && mae <= 0.6) {
      return {
        level: 'good',
        message: 'Good performance. Consider more data or hyperparameter tuning for improvement.',
        color: 'var(--color-warning)',
      };
    } else {
      return {
        level: 'needs-improvement',
        message: 'Model needs improvement. Try more training data or different hyperparameters.',
  ...
  const improvement = useMemo(() => {
    if (!evaluationResult?.comparisonBaseline) return null;
      ...
      <div className="content-header">
        <div>
          ...
          <p className="content-subtitle">
            Review your model's performance on the held-out test set.
          </p>
        </div>
      </div>
