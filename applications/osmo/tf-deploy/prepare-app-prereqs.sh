...
Commands:
  prepare              Materialize kubeconfig, bootstrap ingress-nginx for nip.io when needed, and register/update Nebius SSO. Default command.
...
prepare_bootstrap_ingress_for_nipio() {
...
will:
  ...
  5. register Nebius SSO using those final hostnames
...
if [[ $# -gt 0 ]]; then
    case "$1" in
        prepare|sync-kubeconfig|register-oidc)
            COMMAND="$1"
            shift
            ;;
        -h|--help)
