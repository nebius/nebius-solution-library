...
variable "slurm_nodeset_login" {
  description = "Configuration of Slurm Login node set. When autoscaling is enabled, its replica bounds override size for both login pods and the dedicated mk8s node group."
  type = object({
    size               = number
