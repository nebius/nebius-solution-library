...
data "local_file" "grafana_access_token" {
  count    = var.o11y.grafana.enabled ? 1 : 0
  filename = "${path.root}/grafana_access_token.txt"

  depends_on = [terraform_data.grafana_access_token]
}
...
resource "nebius_applications_v1alpha1_k8s_release" "grafana" {
  ...
  set = {
    ...
    "grafana.nebius.accessToken" = var.k8s_node_group_sa_id != null ? data.local_file.grafana_access_token[0].content : ""
  }
}
