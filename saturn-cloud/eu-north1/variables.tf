...
variable "h100_infiniband_fabric" {
  description = "InfiniBand fabric for H100 instances (fabric-2, fabric-3, fabric-4, or fabric-6)"
  type        = string
  default     = "fabric-6"

  validation {
    condition     = contains(["fabric-2", "fabric-3", "fabric-4", "fabric-6"], var.h100_infiniband_fabric)
    error_message = "H100 fabric must be one of: fabric-2, fabric-3, fabric-4, fabric-6"
  }
}
