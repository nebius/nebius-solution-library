#!/usr/bin/env python3
"""Render the UID-bound GenMol PodSnapshotContent from captured Pod JSON."""
...
DONOR_PREFIX = "genmol-native-f7-donor-r1-"
IMAGE = (
...
class CaptureError(ValueError):
    """The captured Pod is not the exact ready GenMol donor."""
...
def donor_identity(pod: dict[str, Any]) -> tuple[str, str]:
    ...
    if (
        ...
        or len(containers) != 1
        or containers[0].get("name") != "genmol"
        or containers[0].get("image") != IMAGE
    ):
        raise CaptureError("donor Pod does not contain the exact pinned GenMol container")
