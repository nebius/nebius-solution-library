#!/usr/bin/env python3
"""Tests for UID-bound GenMol capture rendering."""
...
def donor() -> dict:
    return {
        ...
        "kind": "Pod",
        "metadata": {
            "name": "genmol-native-f7-donor-r1-abc12",
            ...
            "labels": {
                ...
                "nvidia.com/snapshot-checkpoint-id": "genmol-native-f7-v1",
            },
            "annotations": {
        ...
        "spec": {
            ...
            "containers": [{"name": "genmol", "image": capture.IMAGE}],
        },
        "status": {
            "phase": "Running",
...
class CaptureRenderTests(unittest.TestCase):
    def test_ready_exact_donor_renders_uid_bound_content(self) -> None:
        ...
        self.assertEqual(source["podRef"]["name"], "genmol-native-f7-donor-r1-abc12")
        self.assertEqual(
            source["podRef"]["uid"],
            "11111111-1111-4111-8111-111111111111",
        )
        self.assertEqual(source["podRef"]["containers"], ["genmol"])
    ...
    def test_wrong_image_is_rejected(self) -> None:
        value = donor()
        value["spec"]["containers"][0]["image"] = "nvcr.io/nim/nvidia/genmol:latest"
