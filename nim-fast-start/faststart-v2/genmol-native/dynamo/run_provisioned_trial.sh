...
readonly expected_validator_sha256="089b4529bd88f0060492699b3c594c6e4557406cd22364e6930d2b44cd588368"
readonly expected_fixture_sha256="3065261de604f495a2fbae1e7fd92488546ee51f2729e5d40e9be5ee2c22f444"
readonly trial_namespace="nim-fast-start"

script_dir=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd -P)
readonly script_dir
readonly contract_path="${script_dir}/restore-interface.live.json"
readonly validator_path="${script_dir}/../validate_genmol.py"
readonly fixture_path="${script_dir}/../fixtures/requests-qed-logp.json"
...
readonly trial_target="genmol-target-${trial_run_id}"
readonly trial_worker="genmol-restore-${trial_run_id}"
readonly trial_probe="genmol-semantic-${trial_run_id}"
readonly trial_canary="genmol-canary-${trial_run_id}"
...
if ! jq -e --arg node "$trial_node" '
  ...
  any(.spec.volumes[]?;
    .name == "artifacts" and
    .persistentVolumeClaim.claimName == "genmol-native-f7-artifacts" and
    .persistentVolumeClaim.readOnly == true) and
  any(.spec.volumes[]?;
    .name == "nim-cache" and
    .persistentVolumeClaim.claimName == "genmol-native-f7-cache" and
...
jq -n \
  ...
  '{
    schema:"archvteams.nebius.ai/genmol-faststart-run/v1",
    demand_at:$demand_at,
    run_id:$run_id,
    target_node:$node,
    ...
    artifact_pvc:"genmol-native-f7-artifacts",
    cache_pvc:"genmol-native-f7-cache"
  }' > "$trial_dir/run.json"
