#!/usr/bin/env python3
"""Static safety checks for rendered GenMol fast-start manifests."""
...
STAGER_SHA256 = "00d66e007609cf4075e76bbb5bbf1787771df98aba3a8bdc1d39195a3a309025"
...
def _check_target(document: dict[str, Any], errors: list[str]) -> None:
    ...
    if not isinstance(annotations, dict):
        annotations = {}
    expected_checkpoint = {
        "direct": "genmol-native-f7-v1",
        "buffered": "genmol-native-f7-v2-buffered",
    ...
    if not isinstance(container, dict):
        ...
        return
    if container.get("name") != "genmol" or container.get("image") != NIM_IMAGE:
        errors.append(f"{location} must use the exact pinned GenMol image")
    ...
    expected_mounts = {
        ...
        ("snapshot-control", "/snapshot-control", "genmol", False),
...
def _check_probe(documents: list[dict[str, Any]], run_id: str, errors: list[str]) -> None:
    ...
    if len(configs) != 1 or len(jobs) != 1:
        ...
        return
    expected_name = f"genmol-semantic-{run_id}"
    config = configs[0]
    job = jobs[0]
    ...
    source = config.get("data", {}).get("validate_genmol.py")
    if not isinstance(source, str) or hashlib.sha256(source.encode("utf-8")).hexdigest() != VALIDATOR_SHA256:
        errors.append("probe ConfigMap does not contain the reviewed strict GenMol validator")
    fixture = config.get("data", {}).get("requests-qed-logp.json")
    if not isinstance(fixture, str) or hashlib.sha256(fixture.encode("utf-8")).hexdigest() != FIXTURE_SHA256:
        errors.append("probe ConfigMap does not contain the pinned QED/LogP GenMol fixture")
    ...
    if (
        ...
        or container.get("image") != stager.get("image")
    ):
        errors.append(f"{location} must use the exact pinned RDKit-capable GenMol image")
    resources = container.get("resources", {})
    for group in ("requests", "limits"):
    ...
    command = container.get("command")
    if command != [PROBE_EXECUTABLE]:
        errors.append(f"{location} must invoke the reviewed GenMol Python executable")
    expected_args = [
        "/validator/validate_genmol.py",
        "--base-url",
        f"http://genmol-canary-{run_id}:8000",
        "--request-file",
        "/validator/requests-qed-logp.json",
        "--receipt-dir",
        "/evidence/semantic",
        "--run-id",
        f"{run_id}-semantic-a",
    ...
    if container.get("args") != expected_args:
        errors.append(f"{location} must execute exactly two fixed strict GenMol probes")
    expected_mounts = {
