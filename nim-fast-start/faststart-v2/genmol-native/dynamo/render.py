#!/usr/bin/env python3
"""Render the offline GenMol native-Dynamo scaffold.
...
RUN_SCHEMA = "archvteams.nebius.ai/genmol-faststart-run/v1"
BINDING_SCHEMA = "archvteams.nebius.ai/genmol-target-binding/v1"
...
def validate_contract(value: dict[str, Any]) -> dict[str, Any]:
    ...
    if probe_image != NIM_IMAGE:
        raise RenderError("contract.probe_image must be the exact pinned RDKit-capable GenMol image")
    probe_executable = _non_placeholder(
        value["probe_executable"], "contract.probe_executable"
    )
    if probe_executable != PROBE_EXECUTABLE:
        raise RenderError("contract.probe_executable must be the reviewed GenMol Python 3 path")
    if _digest(value["validator_sha256"], "contract.validator_sha256") != VALIDATOR_SHA256:
        raise RenderError("contract.validator_sha256 is not the reviewed GenMol validator")
...
def validate_run(value: dict[str, Any]) -> dict[str, Any]:
    ...
    if checkpoint_id != expected_checkpoint:
        raise RenderError("checkpoint_id is not an exact prepared GenMol artifact")
...
def validate_binding(value: dict[str, Any], run: dict[str, Any]) -> dict[str, Any]:
    ...
    if value["container_name"] != "genmol":
        raise RenderError("binding.container_name must be genmol")
    ...
    if normalized_image != NIM_IMAGE:
        raise RenderError("binding.image_id is not the exact pinned GenMol digest")
...
def _base_tokens(run: dict[str, Any], contract: dict[str, Any]) -> dict[str, str]:
    ...
    return {
        ...
        "@@CANARY_SERVICE@@": f"genmol-canary-{run_id}",
        "@@QUALIFIED_SERVICE@@": f"genmol-qualified-{run_id}",
        "@@TARGET_NETWORK_POLICY@@": f"genmol-target-{run_id}",
        "@@PROBE_NETWORK_POLICY@@": f"genmol-probe-{run_id}",
...
def render_probe(
    ...
    validator_path = BASE_DIR.parent / "validate_genmol.py"
    fixture_path = BASE_DIR.parent / "fixtures" / "requests-qed-logp.json"
    try:
        validator_bytes = validator_path.read_bytes()
    except OSError as exc:
        raise RenderError(f"cannot read strict GenMol validator: {exc}") from exc
    import hashlib

    if hashlib.sha256(validator_bytes).hexdigest() != VALIDATOR_SHA256:
        raise RenderError("strict GenMol validator source digest changed")
    try:
        validator_source = validator_bytes.decode("utf-8")
    except UnicodeDecodeError as exc:  # pragma: no cover - repository corruption
        raise RenderError("strict GenMol validator is not UTF-8") from exc
    try:
        fixture_bytes = fixture_path.read_bytes()
        fixture_source = fixture_bytes.decode("utf-8")
    except (OSError, UnicodeDecodeError) as exc:
        raise RenderError(f"cannot read strict GenMol request fixture: {exc}") from exc
    if hashlib.sha256(fixture_bytes).hexdigest() != FIXTURE_SHA256:
        raise RenderError("strict GenMol request fixture digest changed")
    tokens = _base_tokens(run, contract)
    tokens.update(
        {
            "@@TARGET_UID@@": binding["pod_uid"],
