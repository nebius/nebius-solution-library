...
test_tmp=$(mktemp -d /tmp/genmol-provisioned-runner-test.XXXXXX)
readonly test_tmp
trap 'rm -rf -- "$test_tmp"' EXIT

install -d -m 0700 "${test_tmp}/lane/fixtures"
cp -a -- "$source_dynamo_dir" "${test_tmp}/lane/dynamo"
cp -- "${source_dynamo_dir}/../validate_genmol.py" "${test_tmp}/lane/validate_genmol.py"
cp -- "${source_dynamo_dir}/../fixtures/requests-qed-logp.json" "${test_tmp}/lane/fixtures/requests-qed-logp.json"
/usr/bin/jq '
  .release_ready=true |
  .release_blocker="" |
...
export FAKE_HOLDER_NAME="genmol-native-f7-holder-t12-v2"
...
[[ $(/usr/bin/jq -r '.checkpoint_id' "$run_dir/run.json") == "genmol-native-f7-v1" ]] || \
