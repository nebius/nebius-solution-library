...
def approved_contract() -> dict:
    return {
        ...
        "probe_executable": render.PROBE_EXECUTABLE,
...
def run_config() -> dict:
    return {
        ...
        "checkpoint_id": "genmol-native-f7-v1",
        "artifact_version": "1",
        "artifact_manifest_sha256": "5" * 64,
        "artifact_pvc": "genmol-native-artifacts",
        "cache_pvc": "genmol-nim-cache",
    }
...
def target_binding() -> dict:
    return {
        ...
        "pod_name": "genmol-target-ut-a1b2c3",
...
class RenderTests(unittest.TestCase):
    ...
    def test_binding_requires_exact_image_digest(self) -> None:
        bad = target_binding()
        bad["image_id"] = "nvcr.io/nim/nvidia/genmol@sha256:" + "0" * 64
        with self.assertRaisesRegex(render.RenderError, "exact pinned GenMol"):
            render.validate_binding(bad, self.run)
    ...
    def test_buffered_candidate_is_explicit_and_accepts_measured_glibc(self) -> None:
        ...
        candidate["checkpoint_id"] = "genmol-native-f7-v2-buffered"
        self.assertEqual(
    ...
    def test_contract_requires_rdkit_capable_genmol_probe_image(self) -> None:
        bad = approved_contract()
        bad["probe_image"] = "registry.example.invalid/python@sha256:" + "6" * 64
        with self.assertRaisesRegex(render.RenderError, "RDKit-capable GenMol"):
            render.validate_contract(bad)
...
class UnsafeMutationTests(unittest.TestCase):
    ...
    def test_rejects_mutable_image_tag(self) -> None:
        ...
        pod["spec"]["containers"][0]["image"] = "nvcr.io/nim/nvidia/genmol:1.0"
    ...
    def test_rejects_probe_third_request(self) -> None:
        ...
        self.assert_rejected(documents, "exactly two fixed strict GenMol probes")
    ...
    def test_rejects_probe_without_rdkit_capable_genmol_image(self) -> None:
        documents = copy.deepcopy(self.probe)
        job = find_document(documents, "Job", "semantic-probe")
        spec = job["spec"]["template"]["spec"]
        wrong = "registry.example.invalid/python@sha256:" + "6" * 64
        spec["initContainers"][0]["image"] = wrong
        spec["containers"][0]["image"] = wrong
        self.assert_rejected(documents, "RDKit-capable GenMol")

    def test_probe_stages_fixture_as_regular_file_on_t12(self) -> None:
    ...
    def test_rejects_probe_without_stable_t12_affinity(self) -> None:
        documents = copy.deepcopy(self.probe)
        job = find_document(documents, "Job", "semantic-probe")
    ...
    def test_rejects_probe_using_projected_fixture_directly(self) -> None:
        ...
        spec["volumes"][0] = {
            "name": "validator",
            "configMap": {"name": "genmol-semantic-ut-a1b2c3", "defaultMode": 0o444},
        }
