...
OpenFold2, Boltz2, and GenMol completed coherent response-boundary n=3 reruns. Their
...
Their medians are 14.236758, 27.342018, and 12.177434 seconds respectively from
T0 through the second complete response. Historical OpenFold2/Boltz2
response-plus-validation call columns and GenMol validation-complete terminal
values remain unchanged in explicitly legacy evidence and must not be mixed
with the corrected cohorts.

Four stable production-shaped lanes still require an exact-total rerun.
...
All four reruns must retain the existing warm-instance boundaries: Ready GPU
...
GenMol completed this response-boundary requalification; MolMIM and
RFdiffusion were qualified directly under the same contract. None is part of
the remaining rerun list. RFdiffusion's selected
