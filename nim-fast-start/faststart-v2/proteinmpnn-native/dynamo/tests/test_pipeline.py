...
CROSS_LANGUAGE_SPEC_SHA256 = (
    "df573c9d63b52b314c9276a8a356f142c41091887636c541bc7087e5051c9666"
)
...
def evidence_inputs() -> dict:
    ...
    worker_receipt = {
        ...
        "target_name": f"pmp-target-{run_id}",
        "target_uid": POD_UID,
    ...
    semantic = {
        ...
        "endpoint": f"http://{service_name}:8000/biology/ipd/proteinmpnn/predict",
        "inference_path": "/biology/ipd/proteinmpnn/predict",
        ...
        "cases": [
            {
                ...
                "exit_code": 0,
                "elapsed_seconds": 0.8,
                "invariant": {"pdb": {"residue_count": 20}},
            },
            {
                "index": 2,
...
class BindingTests(unittest.TestCase):
    def test_python_matches_go_worker_canonical_pod_spec_fixture(self) -> None:
        fixture_path = TEST_DIR / "fixtures" / "proteinmpnn-api-pod-spec.json"
...
class EvidenceTests(unittest.TestCase):
    ...
    def test_semantic_completion_may_precede_kubernetes_ready(self) -> None:
        inputs = evidence_inputs()
        for condition in inputs["target"]["status"]["conditions"]:
            if condition["type"] == "Ready":
        ...
        self.assertEqual(receipt["status"], "PASS")
        self.assertEqual(
            receipt["timings_seconds"]["semantic_completion_to_kubernetes_ready"],
            0.1,
        )

    def test_semantic_completion_rejects_excessive_kubernetes_ready_lag(self) -> None:
        inputs = evidence_inputs()
        for condition in inputs["target"]["status"]["conditions"]:
            if condition["type"] == "Ready":
                condition["lastTransitionTime"] = "2026-08-17T20:00:20Z"
        with self.assertRaisesRegex(evidence.EvidenceError, "lags semantic completion"):
            evidence.build_evidence(**inputs)

    def test_semantic_evidence_rejects_target_without_ready_condition(self) -> None:
        inputs = evidence_inputs()
        inputs["target"]["status"]["conditions"] = [
            condition
            for condition in inputs["target"]["status"]["conditions"]
            if condition["type"] != "Ready"
        ]
        with self.assertRaisesRegex(evidence.EvidenceError, "successful Ready condition"):
            evidence.build_evidence(**inputs)

    def test_semantic_evidence_rejects_wrong_target_uid(self) -> None:
        inputs = evidence_inputs()
        inputs["target"]["metadata"]["uid"] = "22222222-2222-4222-8222-222222222222"
        with self.assertRaisesRegex(evidence.EvidenceError, "identity does not match"):
            evidence.build_evidence(**inputs)
    ...
    def test_scheduled_timestamp_subsecond_quantization_is_accepted(self) -> None:
        ...
        for condition in inputs["target"]["status"]["conditions"]:
            if condition["type"] == "PodScheduled":
                condition["lastTransitionTime"] = "2026-08-17T20:00:01Z"
        receipt = evidence.build_evidence(**inputs)
        self.assertEqual(receipt["status"], "PASS")
        self.assertEqual(receipt["timings_seconds"]["demand_to_scheduled"], 0.0)
    ...
    def test_ready_timestamp_before_receipt_is_allowed_by_partial_order(self) -> None:
        inputs = evidence_inputs()
        for condition in inputs["target"]["status"]["conditions"]:
            if condition["type"] == "Ready":
