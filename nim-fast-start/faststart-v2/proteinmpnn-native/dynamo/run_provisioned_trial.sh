...
readonly expected_contract_sha256="${PMP_EXPECTED_CONTRACT_SHA256:-f93406d1f54169eb54f733498526b0b1eb4e3c79b225542ac6db84768e2814b4}"
readonly expected_validator_sha256="2e3c21af0987f4b9c7da2cef3f3e4d210a7b223049f231c24e871e2a553b48d3"
readonly trial_namespace="nim-fast-start"

script_dir=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd -P)
readonly script_dir
readonly contract_path="${PMP_CONTRACT_PATH:-${script_dir}/restore-interface.live.json}"
readonly validator_path="${script_dir}/../validate_proteinmpnn.py"
...
readonly trial_target="pmp-target-${trial_run_id}"
readonly trial_worker="pmp-restore-${trial_run_id}"
readonly trial_probe="pmp-semantic-${trial_run_id}"
readonly trial_canary="pmp-canary-${trial_run_id}"
...
jq -n \
  ...
  --arg node "$trial_node" \
  '{
    schema:"archvteams.nebius.ai/proteinmpnn-faststart-run/v1",
    demand_at:$demand_at,
    run_id:$run_id,
    target_node:$node,
    checkpoint_id:"proteinmpnn-native-f7-v3-buffered",
    artifact_version:"1",
    artifact_manifest_sha256:"6a298ceefc93b259e5ec7e6c1e74ae3ab43cdd9a757bee1934923dbfcdc06c07",
    artifact_pvc:"proteinmpnn-native-f7-artifacts",
    cache_pvc:"proteinmpnn-native-f7-cache"
  }' > "$trial_dir/run.json"
