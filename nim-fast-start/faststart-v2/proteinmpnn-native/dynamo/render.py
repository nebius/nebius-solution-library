#!/usr/bin/env python3
"""Render the offline ProteinMPNN native-Dynamo scaffold.
...
RUN_SCHEMA = "archvteams.nebius.ai/proteinmpnn-faststart-run/v1"
BINDING_SCHEMA = "archvteams.nebius.ai/proteinmpnn-target-binding/v1"
...
def validate_contract(value: dict[str, Any]) -> dict[str, Any]:
    ...
    if _digest(value["validator_sha256"], "contract.validator_sha256") != VALIDATOR_SHA256:
        raise RenderError("contract.validator_sha256 is not the reviewed ProteinMPNN validator")
...
def validate_binding(value: dict[str, Any], run: dict[str, Any]) -> dict[str, Any]:
    ...
    if value["container_name"] != "proteinmpnn":
        raise RenderError("binding.container_name must be proteinmpnn")
    ...
    if normalized_image != NIM_IMAGE:
        raise RenderError("binding.image_id is not the exact pinned ProteinMPNN digest")
...
def _base_tokens(run: dict[str, Any], contract: dict[str, Any]) -> dict[str, str]:
    ...
    return {
        ...
        "@@CANARY_SERVICE@@": f"pmp-canary-{run_id}",
        "@@QUALIFIED_SERVICE@@": f"pmp-qualified-{run_id}",
        "@@TARGET_NETWORK_POLICY@@": f"pmp-target-{run_id}",
        "@@PROBE_NETWORK_POLICY@@": f"pmp-probe-{run_id}",
...
def render_probe(
    ...
    validator_path = BASE_DIR.parent / "validate_proteinmpnn.py"
    fixture_path = BASE_DIR.parent / "fixtures" / "1ubq-request.json"
    try:
        validator_bytes = validator_path.read_bytes()
    except OSError as exc:
        raise RenderError(f"cannot read strict ProteinMPNN validator: {exc}") from exc
    import hashlib

    if hashlib.sha256(validator_bytes).hexdigest() != VALIDATOR_SHA256:
        raise RenderError("strict ProteinMPNN validator source digest changed")
    try:
        validator_source = validator_bytes.decode("utf-8")
    except UnicodeDecodeError as exc:  # pragma: no cover - repository corruption
        raise RenderError("strict ProteinMPNN validator is not UTF-8") from exc
    try:
        fixture_bytes = fixture_path.read_bytes()
        fixture_source = fixture_bytes.decode("utf-8")
    except (OSError, UnicodeDecodeError) as exc:
        raise RenderError(f"cannot read strict ProteinMPNN request fixture: {exc}") from exc
    if hashlib.sha256(fixture_bytes).hexdigest() != "42511546f59eb92479149df1fad8d713ae00176b89316b07a83d04586905938e":
        raise RenderError("strict ProteinMPNN request fixture digest changed")
    tokens = _base_tokens(run, contract)
    tokens.update(
        {
            "@@TARGET_UID@@": binding["pod_uid"],
