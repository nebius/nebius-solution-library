#!/usr/bin/env python3
"""Static safety checks for rendered ProteinMPNN fast-start manifests."""
...
NIM_IMAGE = (
    "nvcr.io/nim/ipd/proteinmpnn@sha256:"
    "b55a0aa6733e267e6e6fe06434e98aea61eff14bc5545127555607fef6f38aa5"
)
PROBE_IMAGE = (
...
REQUEST_FIXTURE_SHA256 = "42511546f59eb92479149df1fad8d713ae00176b89316b07a83d04586905938e"
STAGER_SHA256 = "3719af96a711021551b6d4d09e28cd517b8802c8d29d957691bed5b2fba63fff"
...
ALLOWED_H100_NODES = {
    ...
    for item in os.environ.get(
        "ALLOWED_H100_NODES", "gpu-node-a.example.invalid"
    ).split(",")
    if item
}
...
def _check_target(document: dict[str, Any], errors: list[str]) -> None:
    ...
    if not isinstance(container, dict):
        ...
        return
    if container.get("name") != "proteinmpnn" or container.get("image") != NIM_IMAGE:
        errors.append(f"{location} must use the exact pinned ProteinMPNN image")
    ...
    if requests != {"cpu": "10", "memory": "96Gi", "nvidia.com/gpu": "1"}:
    ...
    expected_mounts = {
        ...
        ("snapshot-control", "/snapshot-control", "proteinmpnn", False),
...
def _check_probe(documents: list[dict[str, Any]], run_id: str, errors: list[str]) -> None:
    ...
    if len(configs) != 1 or len(jobs) != 1:
        ...
        return
    expected_name = f"pmp-semantic-{run_id}"
    config = configs[0]
    job = jobs[0]
    ...
    source = config.get("data", {}).get("validate_proteinmpnn.py")
    if not isinstance(source, str) or hashlib.sha256(source.encode("utf-8")).hexdigest() != VALIDATOR_SHA256:
        errors.append("probe ConfigMap does not contain the reviewed strict ProteinMPNN validator")
    fixture = config.get("data", {}).get("1ubq-request.json")
    if not isinstance(fixture, str) or hashlib.sha256(fixture.encode("utf-8")).hexdigest() != REQUEST_FIXTURE_SHA256:
        errors.append("probe ConfigMap does not contain the reviewed strict 1UBQ request")
    ...
    hostnames = _hostname_values(spec)
    if len(hostnames) != 1 or hostnames[0] not in ALLOWED_H100_NODES:
        errors.append(f"{location} must select the exact allowed H100 hostname")
    ...
    if not isinstance(init_containers, list) or len(init_containers) != 1:
        ...
        if (
            ...
            or hashlib.sha256(stager_args[1].encode("utf-8")).hexdigest() != STAGER_SHA256
        ):
    ...
    if container.get("name") != "semantic-probe" or container.get("image") != stager.get("image"):
        errors.append(f"{location} must use the exact pinned CPU probe image")
    resources = container.get("resources", {})
    for group in ("requests", "limits"):
    ...
    expected_args = [
        "/validator/validate_proteinmpnn.py",
        "--base-url",
        f"http://pmp-canary-{run_id}:8000",
        "--receipt-dir",
        "/evidence/semantic",
        "--run-id",
        f"{run_id}-semantic-a",
    ...
    if container.get("args") != expected_args:
        errors.append(f"{location} must execute exactly two fixed strict ProteinMPNN probes")
