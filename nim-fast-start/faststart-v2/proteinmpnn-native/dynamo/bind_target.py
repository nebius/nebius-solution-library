#!/usr/bin/env python3
"""Bind one scheduled ProteinMPNN target to the one-shot restore worker.
...
def build_binding(
    ...
    required_annotations = {
        "nvidia.com/snapshot-target-containers": "proteinmpnn",
    ...
    if (
        container.get("name") != "proteinmpnn"
    ...
    matches = [item for item in statuses if isinstance(item, dict) and item.get("name") == "proteinmpnn"]
    if len(matches) != 1:
        raise BindingError("live Pod does not have one ProteinMPNN container status")
    container_status = matches[0]
    if not isinstance(container_status.get("state", {}).get("running"), dict):
        raise BindingError("live ProteinMPNN placeholder container is not running")
    ...
    if not isinstance(image_id, str) or image_id.removeprefix("docker-pullable://") != render.NIM_IMAGE:
        raise BindingError("live container image ID is not the pinned ProteinMPNN digest")
