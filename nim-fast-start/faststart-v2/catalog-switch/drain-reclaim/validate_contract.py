...
CONTRACT_SCHEMA = "archvteams.nebius.ai/catalog-switch-drain-reclaim-contract/v2"
EXPECTED_SOURCE = {
...
EXPECTED_INVARIANTS = {
    "DR-INV-01": "Admission is open only for one exact runtime after an exact bridge receipt proves two distinct, ordered, raw-evidence-backed semantic calls using the pinned validator source and an independently reread complete off-node durable chain segment.",
    ...
    "DR-INV-04": "The target switch begins only after the shared external-client T0 and every request-specific event remains causally after that immutable acceptance event.",
    ...
    "DR-INV-10": "Exact duplicate reservations and commands are idempotent, conflicting retries are rejected, and both node-local and exact-cluster Kubernetes agents independently refuse double occupancy before physical launch.",
}
EXPECTED_BINDING_REQUIREMENTS = {
    "DR-INV-01": {
        "code": {"DrainReclaimStateMachine.accept_b", "DrainReclaimStateMachine.accept_rollback", "ExactLedgerReceiptVerifier.verify", "ExactLedgerReceiptVerifier._verify_durability"},
        "controls": {"CTL-10", "CTL-19"},
        "tests": {"test_exact_bridge_receipt_is_mandatory_for_admission", "test_semantic_response_loss_reuses_durable_intent_and_idempotency_key", "test_semantic_mismatch_missing_duplicate_reordered_and_prior_calls_reject", "test_validator_source_authority_tamper_rejects"},
    ...
    "DR-INV-03": {
        "code": {"DrainReclaimStateMachine.record_reclaim", "GpuReleaseProof.validate_for"},
        "controls": {"CTL-04", "CTL-05"},
        "tests": {"test_exact_total_absence_before_scrub_and_zero_nvml_are_required", "test_graphics_process_rejects_gpu_release"},
        "threat_tests": {"TST-01", "TST-02"},
    },
    "DR-INV-04": {
        "code": {"DrainReclaimStateMachine.begin_switch", "SwitchLedgerBridge.__init__"},
        "controls": {"CTL-10"},
        "tests": {"test_bridge_refuses_work_before_external_t0"},
        "threat_tests": {"TST-12"},
    },
    "DR-INV-05": {
    ...
    "DR-INV-10": {
        "code": {"DrainReclaimStateMachine.begin_start_b", "FencedActionExecutor.execute", "KubernetesActions.launch", "NodeLocalActions.launch"},
        "controls": {"CTL-19", "CTL-20"},
        "tests": {"test_concurrent_duplicate_b_reserves_one_generation", "test_duplicate_launch_reservation_is_idempotent_and_conflict_rejects", "test_node_local_and_kubernetes_action_adapters_refuse_second_occupancy", "test_kubernetes_adapter_pins_kubeconfig_context_cluster_ca_namespace_node"},
...
def validate(
    ...
    *,
    ...
    if contract.get("schema") != CONTRACT_SCHEMA or contract.get("version") != 2:
        raise ContractError("contract schema/version differs from v2")
    ...
    if invariant_map != EXPECTED_INVARIANTS or len(invariants) != len(EXPECTED_INVARIANTS):
        raise ContractError("invariant identifiers/statements differ from executable v2 semantics")
