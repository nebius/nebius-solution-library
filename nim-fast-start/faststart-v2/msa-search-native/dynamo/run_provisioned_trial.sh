...
readonly expected_validator_sha256="20e8951ceaaa1b81e8129d86b787c6bb009cf2e207d55829cf13f4fa9489188b"
readonly expected_fixture_sha256="874b0e5e3be9776ea289fb46444032e04b63875d9d4110f1560e5435de72686a"
readonly expected_pipe_validator_sha256="29f45a3c0d7197b5ad0757174666b1f6a8e11f2e3dd7cc54d63fc71fb030ad23"
readonly trial_namespace="nim-fast-start"

script_dir=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd -P)
readonly script_dir
readonly contract_path="${script_dir}/restore-interface.live.json"
readonly validator_path="${script_dir}/../validate_msa_search.py"
readonly fixture_path="${script_dir}/../fixtures/request-pdb70.json"
readonly pipe_validator_path="${script_dir}/../verify_mmseqs_pipe.py"
...
actual_pipe_validator_sha256=$(sha256sum "$pipe_validator_path")
actual_pipe_validator_sha256=${actual_pipe_validator_sha256%% *}
if [[ $actual_pipe_validator_sha256 != "$expected_pipe_validator_sha256" ]]; then
...
readonly trial_target="msa-target-${trial_run_id}"
readonly trial_worker="msa-restore-${trial_run_id}"
readonly trial_probe="msa-semantic-${trial_run_id}"
readonly trial_canary="msa-canary-${trial_run_id}"
...
if ! jq -e --arg node "$trial_node" '
  ...
  any(.spec.volumes[]?;
    .name == "artifacts" and
    .persistentVolumeClaim.claimName == "msa-search-native-f7-artifacts" and
    .persistentVolumeClaim.readOnly == true) and
  any(.spec.volumes[]?;
    .name == "nim-cache" and
    .persistentVolumeClaim.claimName == "msa-search-native-f7-cache" and
...
jq -n \
  ...
  '{
    schema:"archvteams.nebius.ai/msa-search-faststart-run/v1",
    demand_at:$demand_at,
    run_id:$run_id,
    target_node:$node,
    ...
    artifact_pvc:"msa-search-native-f7-artifacts",
    cache_pvc:"msa-search-native-f7-cache"
  }' > "$trial_dir/run.json"
...
# The PDB70 payload can be valid even if restored IPC has drifted. Qualify the
# exact retained topology inside the bound target namespace before PASS:
# MMseqs fd 1 and the API worker fd 24 must name one shared pipe.
"${trial_kubectl[@]}" exec -i "$trial_target" -- /usr/bin/python3 - \
  < "$pipe_validator_path" > "$trial_dir/mmseqs-pipe.log"
jq -e -c '
  select(
    .schema == "archvteams.nebius.ai/msa-search-mmseqs-pipe/v1" and
    .status == "PASS" and
    .shared_pipe_verified == true and
    .mmseqs_fd == 1 and
    .api_worker_fd == 24 and
    (.pipe | type == "string")
  )
' "$trial_dir/mmseqs-pipe.log" > "$trial_dir/mmseqs-pipe-receipt.json"
...
python3 "$script_dir/evidence.py" \
  ...
  > "$trial_dir/canary-evidence.base.json"

jq -s '
  .[0] + {mmseqs_pipe: .[1]} |
  select(
    .mmseqs_pipe.status == "PASS" and
    .mmseqs_pipe.shared_pipe_verified == true and
    .mmseqs_pipe.mmseqs_fd == 1 and
    .mmseqs_pipe.api_worker_fd == 24
  )
' "$trial_dir/canary-evidence.base.json" "$trial_dir/mmseqs-pipe-receipt.json" \
  > "$trial_dir/canary-evidence.json"

jq -e 'select(
  .status=="PASS" and
  .request_count==2 and
  .semantic_pass_count==2 and
  .mmseqs_pipe.shared_pipe_verified==true
)' \
...
jq '{run_id,status,timings_seconds,mmseqs_pipe}' "$trial_dir/canary-evidence.json"
