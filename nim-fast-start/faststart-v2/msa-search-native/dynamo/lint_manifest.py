#!/usr/bin/env python3
"""Static safety checks for rendered MSA Search fast-start manifests."""
...
STAGER_SHA256 = "15b555f42af4bbb8d094760c766bf7478eea2c553de85f922ec5d7c367192c20"
...
def _check_target(document: dict[str, Any], errors: list[str]) -> None:
    ...
    if not isinstance(container, dict):
        ...
        return
    if container.get("name") != "msa-search" or container.get("image") != NIM_IMAGE:
        errors.append(f"{location} must use the exact pinned MSA Search image")
    ...
    expected_env = [
        ...
        {"name": "NIM_GLOBAL_MAX_MSA_DEPTH", "value": "500"},
    ]
...
def _check_probe(documents: list[dict[str, Any]], run_id: str, errors: list[str]) -> None:
    ...
    if len(configs) != 1 or len(jobs) != 1:
        ...
        return
    expected_name = f"msa-semantic-{run_id}"
    config = configs[0]
    job = jobs[0]
    ...
    source = config.get("data", {}).get("validate_msa_search.py")
    if not isinstance(source, str) or hashlib.sha256(source.encode("utf-8")).hexdigest() != VALIDATOR_SHA256:
        errors.append("probe ConfigMap does not contain the reviewed strict MSA Search validator")
    fixture = config.get("data", {}).get("request-pdb70.json")
    if not isinstance(fixture, str) or hashlib.sha256(fixture.encode("utf-8")).hexdigest() != FIXTURE_SHA256:
        errors.append("probe ConfigMap does not contain the pinned 76-residue PDB70 fixture")
    ...
    for field in ("hostPID", "hostIPC", "hostNetwork"):
        if spec.get(field) is not False:
            errors.append(f"{location} must set {field}: false")
    hostnames = _hostname_values(spec)
    if hostnames != ["gpu-node-a.example.invalid"]:
    ...
    if not isinstance(containers, list) or len(containers) != 1:
        ...
        return
    container = containers[0]
    if container.get("name") != "semantic-probe" or container.get("image") != stager.get("image"):
        errors.append(f"{location} must use one exact pinned CPU probe image")
    resources = container.get("resources", {})
    for group in ("requests", "limits"):
    ...
    if container.get("args") != expected_args:
        errors.append(f"{location} must execute exactly two fixed strict MSA Search probes")
    expected_mounts = {
