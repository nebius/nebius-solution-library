...
for run_id in run_ids:
    ...
    if (
        ...
        or value.get("mmseqs_pipe", {}).get("shared_pipe_verified") is not True
    ):
...
result = {
    "schema": "archvteams.nebius.ai/msa-search-native-n3/v1",
    "status": "PASS",
...
jq '{status,checkpoint_id,image_io_mode,trial_count,request_count,mmseqs_pipe_pass_count,statistics_seconds}' \
  "$summary_path"
