#!/usr/bin/env python3
"""Render the UID-bound MSA Search PodSnapshotContent from captured Pod JSON."""
...
DONOR_PREFIX = "msa-search-native-f7-donor-r1-"
IMAGE = (
...
class CaptureError(ValueError):
    """The captured Pod is not the exact ready MSA Search donor."""
...
def donor_identity(pod: dict[str, Any]) -> tuple[str, str]:
    ...
    if (
        labels.get("app.kubernetes.io/name") != "msa-search"
        or labels.get("app.kubernetes.io/component") != "checkpoint-donor"
        or labels.get("nvidia.com/snapshot-is-checkpoint-source") != "true"
        or labels.get("nvidia.com/snapshot-checkpoint-id") != "msa-search-native-f7-v1"
        or annotations.get("nvidia.com/snapshot-artifact-version") != "1"
        or annotations.get("nvidia.com/snapshot-target-containers") != "msa-search"
    ...
    if (
        ...
        or len(containers) != 1
        or containers[0].get("name") != "msa-search"
        or containers[0].get("image") != IMAGE
    ):
        raise CaptureError("donor Pod does not contain the exact pinned MSA Search container")
    ...
    if (
        ...
        or len(statuses) != 1
        or statuses[0].get("name") != "msa-search"
