#!/usr/bin/env python3
"""Bind one scheduled DiffDock target to the one-shot restore worker.
...
def build_binding(
    ...
    required_annotations = {
        "nvidia.com/snapshot-target-containers": "diffdock",
    ...
    matches = [item for item in statuses if isinstance(item, dict) and item.get("name") == "diffdock"]
    if len(matches) != 1:
        raise BindingError("live Pod does not have one DiffDock container status")
    container_status = matches[0]
    if not isinstance(container_status.get("state", {}).get("running"), dict):
        raise BindingError("live DiffDock placeholder container is not running")
    ...
    if not isinstance(image_id, str) or image_id.removeprefix("docker-pullable://") != render.NIM_IMAGE:
        raise BindingError("live container image ID is not the pinned DiffDock digest")
