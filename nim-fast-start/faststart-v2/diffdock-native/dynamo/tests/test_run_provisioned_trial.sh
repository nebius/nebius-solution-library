...
test_tmp=$(mktemp -d /tmp/diffdock-provisioned-runner-test.XXXXXX)
readonly test_tmp
...
export FAKE_HOLDER_NAME="diffdock-native-f7-holder-hf93"
...
[[ $(/usr/bin/jq -r '.checkpoint_id' "$run_dir/run.json") == "diffdock-native-f7-v1" ]] || \
