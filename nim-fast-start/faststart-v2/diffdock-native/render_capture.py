#!/usr/bin/env python3
"""Render the UID-bound DiffDock PodSnapshotContent from captured Pod JSON."""
...
class CaptureError(ValueError):
    """The captured Pod is not the exact ready DiffDock donor."""
...
def donor_identity(pod: dict[str, Any]) -> tuple[str, str]:
    ...
    if (
        labels.get("app.kubernetes.io/name") != "diffdock"
        or labels.get("app.kubernetes.io/component") != "checkpoint-donor"
        or labels.get("nvidia.com/snapshot-is-checkpoint-source") != "true"
        or labels.get("nvidia.com/snapshot-checkpoint-id") != "diffdock-native-f7-v1"
        or annotations.get("nvidia.com/snapshot-artifact-version") != "1"
        or annotations.get("nvidia.com/snapshot-target-containers") != "diffdock"
    ...
    if (
        ...
        or len(containers) != 1
        or containers[0].get("name") != "diffdock"
        or containers[0].get("image") != IMAGE
    ):
        raise CaptureError("donor Pod does not contain the exact pinned DiffDock container")
    ...
    if (
        ...
        or len(statuses) != 1
        or statuses[0].get("name") != "diffdock"
