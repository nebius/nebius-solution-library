#!/usr/bin/env python3
"""Tests for UID-bound DiffDock capture rendering."""
    ...
    return {
        ...
        "metadata": {
            ...
            "labels": {
                ...
                "nvidia.com/snapshot-checkpoint-id": "diffdock-native-f7-v1",
            },
            "annotations": {
        ...
        "spec": {
            ...
            "containers": [{"name": "diffdock", "image": capture.IMAGE}],
        },
        "status": {
            "phase": "Running",
...
class CaptureRenderTests(unittest.TestCase):
    ...
    def test_wrong_image_is_rejected(self) -> None:
        value = donor()
        value["spec"]["containers"][0]["image"] = "nvcr.io/nim/mit/diffdock:latest"
