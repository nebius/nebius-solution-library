#!/usr/bin/env python3
"""Bind one scheduled OpenFold2 target to the one-shot restore worker.
...
def build_binding(
    ...
    required_annotations = {
        "nvidia.com/snapshot-target-containers": "openfold2",
    ...
    matches = [item for item in statuses if isinstance(item, dict) and item.get("name") == "openfold2"]
    if len(matches) != 1:
        raise BindingError("live Pod does not have one OpenFold2 container status")
    container_status = matches[0]
    if not isinstance(container_status.get("state", {}).get("running"), dict):
        raise BindingError("live OpenFold2 placeholder container is not running")
    ...
    if not isinstance(image_id, str) or image_id.removeprefix("docker-pullable://") != render.NIM_IMAGE:
        raise BindingError("live container image ID is not the pinned OpenFold2 digest")
