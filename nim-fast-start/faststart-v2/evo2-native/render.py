#!/usr/bin/env python3
"""Render Evo2 target, one-shot restore worker, and early semantic probe."""
...
base = _load_module("openfold2_dynamo_render_for_evo2", BASE_DYNAMO / "render.py")

NAMESPACE = "nim-fast-start"
RUN_SCHEMA = "archvteams.nebius.ai/evo2-faststart-run/v1"
BINDING_SCHEMA = "archvteams.nebius.ai/evo2-target-binding/v1"
VALIDATOR_SHA256 = "113747668c537e83c61648e193419248696e144d5b0598b1f6ca7d38efabe496"
...
def validate_run(value: dict[str, Any]) -> dict[str, Any]:
    ...
    if value.get("target_node") != TARGET_NODE:
        raise RenderError("target_node is not the exact retained single-H200 node")
...
def validate_binding(value: dict[str, Any], run: dict[str, Any]) -> dict[str, Any]:
    ...
    for key, item in expected.items():
        if value.get(key) != item:
            raise RenderError(f"binding {key} does not match the exact Evo2 run")
...
def _base_tokens(run: dict[str, Any], contract: dict[str, Any]) -> dict[str, str]:
    ...
    return {
        ...
        "@@CANARY_SERVICE@@": f"e2-canary-{run_id}",
        "@@QUALIFIED_SERVICE@@": f"e2-qualified-{run_id}",
        "@@TARGET_NETWORK_POLICY@@": f"e2-target-{run_id}",
        "@@PROBE_NETWORK_POLICY@@": f"e2-probe-{run_id}",
        "@@WORKER_NAME@@": f"e2-restore-{run_id}",
        "@@WORKER_IMAGE@@": contract["worker_image"],
        "@@WORKER_EXECUTABLE@@": contract["worker_executable"],
        "@@PROBE_NAME@@": f"e2-semantic-{run_id}",
...
def _evo_structure(value: Any) -> Any:
    if isinstance(value, dict):
        return {_evo_structure(key): _evo_structure(item) for key, item in value.items()}
    if isinstance(value, list):
        return [_evo_structure(item) for item in value]
    if isinstance(value, str):
        if value == OPENFOLD_IMAGE:
            return NIM_IMAGE
        return (
            value.replace("validate_openfold2", "validate_evo2")
            .replace("OpenFold2", "Evo2")
            .replace("openfold2", "evo2")
            .replace("of2-", "e2-")
        )
    return value
...
def _load_template(name: str) -> list[dict[str, Any]]:
    try:
        return _evo_structure(base._load_template(name))
    except base.RenderError as exc:
        raise RenderError(str(exc)) from exc
...
def render_probe(
    ...
    source = (HERE / "validate_evo2.py").read_bytes()
    if hashlib.sha256(source).hexdigest() != VALIDATOR_SHA256:
        raise RenderError("strict Evo2 validator source digest changed")
    tokens = _base_tokens(run, contract)
    tokens.update(
        {
            "@@TARGET_UID@@": binding["pod_uid"],
...
def lint_documents(documents: list[dict[str, Any]]) -> list[str]:
        ...
        if len(pods) != 1:
            ...
            if len(containers) != 1:
                ...
                if container.get("resources") != {
                    ...
                    errors.append("target resources do not match the H200 profile")
                cache_mounts = [
            ...
            if "nodeName" in spec or _hostname_affinity(spec) != [TARGET_NODE]:
                errors.append("target must use scheduler affinity for the exact H200 node")
            ...
            if len(containers) != 1:
                ...
                if component == "semantic-probe":
                    ...
                    if "--base-url" not in args or not any(
                        isinstance(item, str) and item.startswith("http://e2-canary-")
                        for item in args
                    ):
    ...
    if config_maps:
        if len(config_maps) != 1:
            ...
            source = config.get("data", {}).get("validate_evo2.py")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"render-evo2: refused: {exc}", file=sys.stderr)
        return 2
