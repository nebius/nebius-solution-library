#!/usr/bin/env python3
"""Render the offline MolMIM native-Dynamo scaffold.
...
RUN_SCHEMA = "archvteams.nebius.ai/molmim-faststart-run/v1"
BINDING_SCHEMA = "archvteams.nebius.ai/molmim-target-binding/v1"
...
def validate_contract(value: dict[str, Any]) -> dict[str, Any]:
    ...
    if probe_image != NIM_IMAGE:
        raise RenderError(
            "contract.probe_image must be the exact pinned RDKit-capable MolMIM image"
        )
    probe_executable = _non_placeholder(
        value["probe_executable"], "contract.probe_executable"
    )
    if probe_executable != PROBE_EXECUTABLE:
        raise RenderError(
            "contract.probe_executable must be the reviewed MolMIM Python 3 path"
        )
    if _digest(value["validator_sha256"], "contract.validator_sha256") != VALIDATOR_SHA256:
        raise RenderError("contract.validator_sha256 is not the reviewed MolMIM validator")
...
def validate_compatibility(
    ...
    """Reject runs outside the worker's declared artifact/runtime envelope."""
    ...
    target_version = tuple(int(part) for part in run["target_glibc_version"].split("."))
    required_version = tuple(
        int(part)
        for part in contract["tool_bundle"]["maximum_required_glibc"].split(".")
    )
    if target_version < required_version:
        raise RenderError(
            "target_glibc_version is older than the worker tool bundle requirement"
        )
    return run, contract
...
def validate_run(value: dict[str, Any]) -> dict[str, Any]:
    ...
    if checkpoint_id != expected_checkpoint:
        raise RenderError("checkpoint_id is not an exact prepared MolMIM artifact")
...
def validate_binding(value: dict[str, Any], run: dict[str, Any]) -> dict[str, Any]:
    ...
    if value["container_name"] != "molmim":
        raise RenderError("binding.container_name must be molmim")
    ...
    if normalized_image != NIM_IMAGE:
        raise RenderError("binding.image_id is not the exact pinned MolMIM digest")
...
def _base_tokens(run: dict[str, Any], contract: dict[str, Any]) -> dict[str, str]:
    ...
    return {
        ...
        "@@CANARY_SERVICE@@": f"molmim-canary-{run_id}",
        "@@QUALIFIED_SERVICE@@": f"molmim-qualified-{run_id}",
        "@@TARGET_NETWORK_POLICY@@": f"molmim-target-{run_id}",
        "@@PROBE_NETWORK_POLICY@@": f"molmim-probe-{run_id}",
...
def render_probe(
    ...
    validator_path = BASE_DIR.parent / "validate_molmim.py"
    fixture_path = BASE_DIR.parent / "fixtures" / "request-cmaes-qed.json"
    try:
        validator_bytes = validator_path.read_bytes()
    except OSError as exc:
        raise RenderError(f"cannot read strict MolMIM validator: {exc}") from exc
    import hashlib

    if hashlib.sha256(validator_bytes).hexdigest() != VALIDATOR_SHA256:
        raise RenderError("strict MolMIM validator source digest changed")
    try:
        validator_source = validator_bytes.decode("utf-8")
    except UnicodeDecodeError as exc:  # pragma: no cover - repository corruption
        raise RenderError("strict MolMIM validator is not UTF-8") from exc
    try:
        fixture_bytes = fixture_path.read_bytes()
        fixture_source = fixture_bytes.decode("utf-8")
    except (OSError, UnicodeDecodeError) as exc:
        raise RenderError(f"cannot read strict MolMIM request fixture: {exc}") from exc
    if hashlib.sha256(fixture_bytes).hexdigest() != FIXTURE_SHA256:
        raise RenderError("strict MolMIM request fixture digest changed")
    tokens = _base_tokens(run, contract)
    tokens.update(
        {
            "@@TARGET_UID@@": binding["pod_uid"],
