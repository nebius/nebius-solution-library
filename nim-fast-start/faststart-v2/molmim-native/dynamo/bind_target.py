#!/usr/bin/env python3
"""Bind one scheduled MolMIM target to the one-shot restore worker.
...
def _require_expected(actual: Any, expected: Any, label: str) -> None:
    """Require rendered fields while allowing API defaults and scheduler fields."""
    if isinstance(expected, dict):
        if not isinstance(actual, dict):
            raise BindingError(f"{label} is not an object")
        for key, expected_value in expected.items():
            if key not in actual:
                raise BindingError(f"{label}.{key} is absent")
            _require_expected(actual[key], expected_value, f"{label}.{key}")
        return
    if isinstance(expected, list):
        if not isinstance(actual, list) or len(actual) != len(expected):
            raise BindingError(f"{label} list does not match the rendered target")
        ...
        return
    if actual != expected:
        raise BindingError(f"{label} does not match the rendered target")
...
def build_binding(
    ...
    required_annotations = {
        "nvidia.com/snapshot-target-containers": "molmim",
    ...
    expected_target = next(
        document
        for document in render.render_target(run, contract)
        if document.get("kind") == "Pod"
    )
    _require_expected(metadata, expected_target["metadata"], "live Pod metadata")
    expected_spec = dict(expected_target["spec"])
    # These three core bools are not pointers and disappear from an API GET
    # when false even though the submitted manifest explicitly sets them.
    for omitted_false in ("hostIPC", "hostNetwork", "hostPID"):
        if expected_spec.get(omitted_false) is False:
            del expected_spec[omitted_false]
    _require_expected(spec, expected_spec, "live Pod spec")
    ...
    matches = [item for item in statuses if isinstance(item, dict) and item.get("name") == "molmim"]
    if len(matches) != 1:
        raise BindingError("live Pod does not have one MolMIM container status")
    container_status = matches[0]
    if not isinstance(container_status.get("state", {}).get("running"), dict):
        raise BindingError("live MolMIM placeholder container is not running")
    ...
    if not isinstance(image_id, str) or image_id.removeprefix("docker-pullable://") != render.NIM_IMAGE:
        raise BindingError("live container image ID is not the pinned MolMIM digest")
