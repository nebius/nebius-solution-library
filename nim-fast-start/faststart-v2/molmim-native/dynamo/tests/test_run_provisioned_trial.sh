...
test_tmp=$(mktemp -d /tmp/molmim-provisioned-runner-test.XXXXXX)
readonly test_tmp
trap 'rm -rf -- "$test_tmp"' EXIT

install -d -m 0700 "${test_tmp}/lane/fixtures"
cp -a -- "$source_dynamo_dir" "${test_tmp}/lane/dynamo"
cp -- "${source_dynamo_dir}/../validate_molmim.py" "${test_tmp}/lane/validate_molmim.py"
cp -- "${source_dynamo_dir}/../fixtures/request-cmaes-qed.json" "${test_tmp}/lane/fixtures/request-cmaes-qed.json"
/usr/bin/jq '
  .release_ready=true |
  .release_blocker="" |
...
export FAKE_IMAGE_HOLDER_READY=true
...
old_glibc_args=("${runner_args[@]}")
for ((argument_index = 0; argument_index < ${#old_glibc_args[@]}; argument_index++)); do
  if [[ ${old_glibc_args[$argument_index]} == "--target-glibc-version" ]]; then
    old_glibc_args[argument_index + 1]=2.34
  fi
done
: > "$FAKE_CALL_LOG"
if "$runner" --run-id old-glibc "${old_glibc_args[@]}" \
  > "${test_tmp}/old-glibc.stdout" 2> "${test_tmp}/old-glibc.stderr"; then
  fail "runner accepted a target glibc older than the worker bundle"
fi
[[ ! -s $FAKE_CALL_LOG ]] || fail "old glibc guard reached kubectl"
rg -q 'older than the worker tool bundle requirement 2.35' \
  "${test_tmp}/old-glibc.stderr" || fail "old glibc refusal was not explicit"
pass "target glibc older than the worker bundle fails before Kubernetes"
...
: > "$FAKE_CALL_LOG"
export FAKE_HOLDER_READY=true
export FAKE_CACHE_HOLDER_READY=false
if "$runner" --run-id cache-not-ready "${runner_args[@]}" \
  > "${test_tmp}/cache-holder.stdout" 2> "${test_tmp}/cache-holder.stderr"; then
  fail "runner accepted a non-Ready cache holder"
fi
[[ $(<"$FAKE_CALL_LOG") == $'config-view\ncache-holder-get' ]] || \
  fail "cache holder precondition made extra calls"
[[ ! -e ${test_tmp}/evidence/runs/cache-not-ready ]] || fail "cache holder failure created evidence"
pass "Ready same-node fully prewarmed cache holder is a pre-mutation condition"

: > "$FAKE_CALL_LOG"
export FAKE_HOLDER_READY=true
export FAKE_CACHE_HOLDER_READY=true
export FAKE_IMAGE_HOLDER_READY=false
if "$runner" --run-id image-not-ready "${runner_args[@]}" \
  > "${test_tmp}/image-holder.stdout" 2> "${test_tmp}/image-holder.stderr"; then
  fail "runner accepted a non-Ready exact-image holder"
fi
[[ $(<"$FAKE_CALL_LOG") == $'config-view\ncache-holder-get\nimage-holder-get' ]] || \
  fail "image holder precondition made extra calls"
[[ ! -e ${test_tmp}/evidence/runs/image-not-ready ]] || fail "image holder failure created evidence"
pass "Ready same-node exact-image holder is a pre-mutation condition"
...
[[ $(/usr/bin/jq -r '.checkpoint_id' "$run_dir/run.json") == "molmim-native-f7-v1" ]] || \
