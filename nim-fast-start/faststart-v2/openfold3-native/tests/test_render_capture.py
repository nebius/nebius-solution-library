#!/usr/bin/env python3
"""Tests for UID-bound OpenFold3 capture rendering."""
...
def donor() -> dict:
    return {
        ...
        "kind": "Pod",
        "metadata": {
            "name": "openfold3-native-f7-donor-r3-abc12",
            ...
            "labels": {
                ...
                "nvidia.com/snapshot-checkpoint-id": "openfold3-native-f7-v1",
            },
            "annotations": {
                "nvidia.com/snapshot-artifact-version": "1",
                "nvidia.com/snapshot-target-containers": "openfold3",
        ...
        "spec": {
            ...
            "containers": [{"name": "openfold3", "image": capture.IMAGE}],
        },
        "status": {
            "phase": "Running",
...
class CaptureRenderTests(unittest.TestCase):
    def test_ready_exact_donor_renders_uid_bound_content(self) -> None:
        ...
        self.assertEqual(source["podRef"]["name"], "openfold3-native-f7-donor-r3-abc12")
        self.assertEqual(
            source["podRef"]["uid"],
            "11111111-1111-4111-8111-111111111111",
        )
        self.assertEqual(source["podRef"]["containers"], ["openfold3"])

    def test_wrong_node_is_rejected(self) -> None:
        value = donor()
        value["spec"]["nodeName"] = "computeinstance-e00different"
    ...
    def test_wrong_image_is_rejected(self) -> None:
        value = donor()
        value["spec"]["containers"][0]["image"] = "nvcr.io/nim/openfold/openfold3:latest"
