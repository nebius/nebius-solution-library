#!/usr/bin/env python3
"""Render a write-once direct-to-buffered OpenFold3 artifact variant Job."""
...
SOURCE_ID = "openfold3-native-f7-v1"
DESTINATION_ID = "openfold3-native-f7-v2-buffered"
NODE = "gpu-node-a.example.invalid"
PVC = "openfold3-native-f7-artifacts"
PYTHON_IMAGE = (
...
def read_receipt(path: Path) -> dict[str, Any]:
    ...
    if (
        value["schema"] != "archvteams.nebius.ai/openfold3-native-artifact-receipt/v1"
...
def build_script(receipt: dict[str, Any]) -> str:
...
try:
    ...
    if digest((SOURCE / "manifest.yaml").read_bytes()) != EXPECTED_MANIFEST_SHA256:
...
print(json.dumps({{
    "schema": "archvteams.nebius.ai/openfold3-buffered-build/v1",
    "status": "PASS",
...
def render(receipt: dict[str, Any]) -> list[dict[str, Any]]:
    labels = {
        "app.kubernetes.io/name": "openfold3",
    ...
    job = {
        ...
        "kind": "Job",
        "metadata": {
            "name": "openfold3-native-f7-v2-buffered-build",
