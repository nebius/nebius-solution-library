#!/usr/bin/env python3
"""Bind one scheduled OpenFold3 target to the one-shot restore worker.
...
def build_binding(
    ...
    matches = [item for item in statuses if isinstance(item, dict) and item.get("name") == "openfold3"]
    if len(matches) != 1:
        raise BindingError("live Pod does not have one OpenFold3 container status")
    container_status = matches[0]
    if not isinstance(container_status.get("state", {}).get("running"), dict):
        raise BindingError("live OpenFold3 placeholder container is not running")
    ...
    if not isinstance(image_id, str) or image_id.removeprefix("docker-pullable://") != render.NIM_IMAGE:
        raise BindingError("live container image ID is not the pinned OpenFold3 digest")
