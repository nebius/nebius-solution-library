...
test_tmp=$(mktemp -d /tmp/openfold3-provisioned-runner-test.XXXXXX)
readonly test_tmp
trap 'rm -rf -- "$test_tmp"' EXIT

install -d -m 0700 "${test_tmp}/lane/fixtures"
cp -a -- "$source_dynamo_dir" "${test_tmp}/lane/dynamo"
cp -- "${source_dynamo_dir}/../validate_openfold3.py" "${test_tmp}/lane/validate_openfold3.py"
cp -- "${source_dynamo_dir}/../fixtures/request-20aa.json" "${test_tmp}/lane/fixtures/request-20aa.json"
/usr/bin/jq '
  .release_ready=true |
  .release_blocker="" |
...
export FAKE_HOLDER_NAME="openfold3-native-f7-holder-hf93"
...
[[ $(/usr/bin/jq -r '.checkpoint_id' "$run_dir/run.json") == "openfold3-native-f7-v1" ]] || \
