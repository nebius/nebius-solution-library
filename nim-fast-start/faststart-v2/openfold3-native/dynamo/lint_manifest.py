#!/usr/bin/env python3
"""Static safety checks for rendered OpenFold3 fast-start manifests."""
...
STAGER_SHA256 = "46081734b35b562cd675c0e6a5289e5a2041f75e170486e56c46ec24a4e65d74"
...
def _check_target(document: dict[str, Any], errors: list[str]) -> None:
    ...
    if not isinstance(container, dict):
        ...
        return
    if container.get("name") != "openfold3" or container.get("image") != NIM_IMAGE:
        errors.append(f"{location} must use the exact pinned OpenFold3 image")
    ...
    if requests != {"cpu": "10", "memory": "120Gi", "nvidia.com/gpu": "1"}:
        errors.append(f"{location} target requests do not match the capture envelope")
    if limits != {"cpu": "11", "memory": "180Gi", "nvidia.com/gpu": "1"}:
...
def _check_probe(documents: list[dict[str, Any]], run_id: str, errors: list[str]) -> None:
    ...
    if len(configs) != 1 or len(jobs) != 1:
        ...
        return
    expected_name = f"of3-semantic-{run_id}"
    config = configs[0]
    job = jobs[0]
    ...
    source = config.get("data", {}).get("validate_openfold3.py")
    if not isinstance(source, str) or hashlib.sha256(source.encode("utf-8")).hexdigest() != VALIDATOR_SHA256:
        errors.append("probe ConfigMap does not contain the reviewed strict OpenFold3 validator")
    fixture = config.get("data", {}).get("request-20aa.json")
    if not isinstance(fixture, str) or hashlib.sha256(fixture.encode("utf-8")).hexdigest() != FIXTURE_SHA256:
        errors.append("probe ConfigMap does not contain the pinned 20-aa OpenFold3 fixture")
    ...
    expected_args = [
        "/validator/validate_openfold3.py",
        "--base-url",
        f"http://of3-canary-{run_id}:8000",
        "--request-file",
        "/validator/request-20aa.json",
        "--receipt-dir",
        "/evidence/semantic",
        "--run-id",
        f"{run_id}-semantic-a",
    ...
    if container.get("args") != expected_args:
        errors.append(f"{location} must execute exactly two fixed strict OpenFold3 probes")
    expected_mounts = {
