# Conventional NIM startup baselines

Cold-start and warm-cache timing baselines for two NVIDIA NIM containers on Nebius Managed Kubernetes.
...
> Note: another GPU experiment interrupted warm Run 5, so four warm runs were recorded.
...
- Four steady-state warm runs were sufficient for baseline characterization.
...
Any startup optimization should improve on these values:

| NIM | Mode | Baseline startup_s (p50) | Improvement target |
|---|---|---|---|
| OpenFold2 | cold | 114 | < 114 s |
| OpenFold2 | warm | 109 | < 109 s |
| Evo2-40B | cold | 612 | < 612 s |
| Evo2-40B | warm | 158 | < 158 s |

Evo2-40B cold start is dominated by model download and initialization, so it offers the largest opportunity for cache pre-seeding.
