...
# Generate file list (only run this once)
if [[ ! -f $FILE_LIST ]]; then
...
for i in "${!NODES[@]}"; do
 ...
 #   scp -o StrictHostKeyChecking=no "$PART_FILE" "$NODE:$REMOTE_WORK_DIR/"
    ...
    ssh -o StrictHostKeyChecking=no "$NODE" "mkdir -p $REMOTE_WORK_DIR && \
                 ...
                 > $REMOTE_WORK_DIR/rclone_progress.log 2>&1 &" &

    # Fetch logs in the background
    nohup ssh -o StrictHostKeyChecking=no "$NODE" "tail -f $REMOTE_WORK_DIR/rclone_progress.log" > "$LOG_FILE" 2>&1 &
done
