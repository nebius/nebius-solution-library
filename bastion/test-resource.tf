locals {
  test_wg_host = trimsuffix(nebius_compute_v1_instance.bastion_instance.status.network_interfaces[0].public_ip_address.address, "/32")
}
...
resource "null_resource" "check_wireguard_web_ui" {
  depends_on = [null_resource.check_bastion_instance]
  count      = var.test_mode ? 1 : 0
