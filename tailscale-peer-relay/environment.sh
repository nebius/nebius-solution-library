...
NEBIUS_VPC_SUBNET_ID=$(nebius vpc subnet list \
  ...
  --format json \
  | jq -r '.items[1].metadata.id')
export NEBIUS_VPC_SUBNET_ID

# Object Storage Bucket
export NEBIUS_BUCKET_NAME="tfstate-tailscale-peer-relay-$(echo -n "${NEBIUS_TENANT_ID}-${NEBIUS_PROJECT_ID}" | md5sum | awk '$0=$1')"
...
NEBIUS_SA_NAME="tailscale-peer-relay-tfstate-sa"
...
#Create mysterybox-payload-viewer group - TODO: move it to terraform code when supported
MYSTERYBOX_PAYLOAD_VIEWER_GROUP=$(nebius iam group get-by-name \
  --parent-id "${NEBIUS_TENANT_ID}" \
  --name 'mysterybox-payload-viewer' \
  --format json \
  | jq -r '.metadata.id')
if [ -z "${MYSTERYBOX_PAYLOAD_VIEWER_GROUP}" ]; then
  export MYSTERYBOX_PAYLOAD_VIEWER_GROUP=$(nebius iam group create \
   --parent-id "${NEBIUS_TENANT_ID}" \
   --name mysterybox-payload-viewer)
else
   export MYSTERYBOX_PAYLOAD_VIEWER_GROUP
fi

PAYLOAD_ACCESS_EXIST=$(nebius iam access-permit list \
    --parent-id "${MYSTERYBOX_PAYLOAD_VIEWER_GROUP}" \
    --format json |
  jq -r --arg project_id "$NEBIUS_PROJECT_ID" '
    .items[]?
    | select(
        .spec.role == "mysterybox.payload-viewer" and
        .spec.resource_id == $project_id
      )
    | "\(.spec.resource_id)"
  '
)
if [ -z "${PAYLOAD_ACCESS_EXIST}" ]; then
  PAYLOAD_VIEWER_ACCESS_BINDING=$(nebius iam access-permit create \
    --parent-id "${MYSTERYBOX_PAYLOAD_VIEWER_GROUP}" \
    --resource-id "${NEBIUS_PROJECT_ID}" \
    --role mysterybox.payload-viewer )
else
  echo "Role already assigned for the group: ${MYSTERYBOX_PAYLOAD_VIEWER_GROUP}"
fi
...
NEBIUS_SA_ACCESS_KEY_ID=$(nebius iam v2 access-key create \
  --parent-id "${NEBIUS_PROJECT_ID}" \
  --name "tailscale-peer-relay-tfstate-$(date +%s)" \
...
terraform {
  backend "s3" {
    ...
    key    = "tailscale-peer-relay.tfstate"
